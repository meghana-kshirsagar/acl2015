...
The FrameNet representation describes events and other scenarios 
with an inventory of \textbf{frames} (\nss{examples}). 
Each frame in is associated with lexical \textbf{predicates} (verbs, nouns, adjectives, and adverbs) capable of 
...
with roles.\nss{example}
FrameNet defines a structured hierarchy of over 1,000 frames associated with \nss{\#}~English lexical predicates, 
...
But a full \nss{\#}\% of sentences in FrameNet---the lexicographic \textbf{exemplars}---are annotated for only one frame, 
and have thus far not been exploited successfully for frame-semantic parsing.
Here, we seek to leverage FrameNet's partially-annotated exemplar sentences 
...
Another resource, PropBank \citep{propbank}, has been widely used for SRL \citep{palmer-10}. 
