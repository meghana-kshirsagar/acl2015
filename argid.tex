...
Our best model achieves a 3.0 increase in F1, or 7.5\% reduction in error, over state-of-the-art. \st{can you say that with F1?}
\end{abstract}
...
The subset of exemplar sentences containing argument annotations is described in \cref{tbl:datastats}.
...
is not a concern. However, the rate of overt arguments per frame 
is noticeably higher for exemplars, which is a cause for concern as it might bias the model's 
...
This second evaluation, we believe, gives a useful indication of the robustness of the SRL model.
...
For the FT annotations, we use the same FrameNet 1.5 data and train/test splits as \cite{das-14}, without any of our own processing.


\nss{tuning regularizer for all experiments}
We tune the $\ell_2$ regularization parameter $\lambda$ on the FT dev set. We searched over the following
...
to only tune parameters and not part of the model construction. Also note that we do not tune any parameters w.r.t the exemplars data.
...
& \multicolumn{1}{c}{\hspace{40pt}\textit{train\footnote{We do not include the dev set in these statistics, which had 4463 frames over 746 sentences.}}} & \multicolumn{1}{c}{\textit{test}} & \multicolumn{1}{c}{\textit{train}} & \multicolumn{1}{c}{\textit{test}} \\
\midrule
