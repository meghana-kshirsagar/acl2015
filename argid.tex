...
We treat the argument identification task as a structured prediction problem.
...
\begin{equation}
\text{score}_\mathbf{w}(a \mid \mathbf{x}, p, f, r) = \mathbf{w}^\top \mathbf{\phi}(a, \mathbf{x}, p, f, r)
\end{equation}
$\text{score}_\mathbf{w}$ models the compatibility  of a candidate role--argument pair.
Its parameters (feature weights) $\mathbf{w}$ are learned from data (\S\ref{sec:learning}).
...
Note that in many cases the role will be empty (non-overt), denoted $\emptyset$. 
...
The global classifier chooses a joint assignment of all arguments of a frame that maximizes the sum of scores under the model, while satisfying the following constraints:
\begin{enumerate}
  \item a role may be assigned to at most one span, and
  \item spans of overt arguments must not overlap.
\end{enumerate}
More concretely, let $\mathbf{a} = \{ (r, a) \mid r \in \text{roles}(f) \}$ be a joint assigment, and let $A$ be the set of all such assignments.
We give $\mathbf{a}$ the score
\begin{equation}
\text{score}_{\mathbf{w}}(\mathbf{a} \mid \mathbf{x}, p, f) =
    \sum_{(r, a) \in \mathbf{a}}{\text{score}_\mathbf{x}(a \mid \mathbf{x}, p, f, r)}
\end{equation}
And choose the joint assignment
\begin{equation}
\textit{args}(\mathbf{x}, p, f) =
    \argmax_{\mathbf{a} \in A} \text{score}_{\mathbf{w}}(\mathbf{a} \mid \mathbf{x}, p, f, r)
\end{equation}
Beam search, with a beam size of 100, is used to choose the maximum joint assignment with no overlapping arguments.
\st{something about DP in Täckström et al, TACL}
