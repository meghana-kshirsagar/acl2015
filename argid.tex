...
\subsection{Full-text Annotations}
...
In some cases, this involved creating a new frame or adding a new lexical unit to an existing frame. 
...
While performance is not significantly affected\st{we checked this, right?}\nss{meghana should have P/R/F and runtime numbers}, these changes enabled us to run more experiments with the larger exemplar dataset and expanded feature space. 
...
But at test time, all frame elements for a given frame are decoded at once, and beam search is used to ensure that no roles\nss{arguments?} overlap.\nss{does beam search require normalizing to probabilities?}

\nss{what preprocessing? MSTParser?}
