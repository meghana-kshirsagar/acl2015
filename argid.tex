...
We tune the $\ell_2$ regularization parameter $\lambda$ on the FT held-out set. We searched over the following
values: 10$^{-5}$, 10$^{-7}$, 10$^{-9}$, 10$^{-12}$ (note that our loss is normalized).
We also use the performance on the FT held-out set to determine the stopping criterion for the stochastic optimization.
We do not tune any parameters w.r.t the exemplars data.
