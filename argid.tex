...
\maketitle
\begin{abstract}
The sparseness of annotated data is a major obstacle 
to automating semantic analysis with broad coverage.
Yet separate datasets annotated with similar forms of annotation
potentially provide weak evidence for the task of interest.
This paper considers the argument identification subtask 
of frame-semantic parsing, which to date has relied exclusively 
upon full-text annotations in the FrameNet resource.
We propose to augment supervised learning with additional 
``indirect'' training data and features 
so as to leverage additional resources internal and external to FrameNet. 
\nss{result}
\end{abstract}
