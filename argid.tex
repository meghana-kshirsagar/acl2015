...
Let $(r^{(i)}, a^{(i)})$ be the $i^{\text{th}}$ training example.
Then the structured hinge loss on the $i^{\text{th}}$ example is given by:
\begin{align*}
\lefteqn{\textit{Hinge}_\mathbf{w}(i) =} \\
&&\max_r{\{\mathbf{w}^\top \mathbf{\phi}(a^{(i)}, \mathbf{x}, p, f, r) + \text{cost}(r, r^{(i)})\}} - \\
&&\mathbf{w}^\top \mathbf{\phi}(a^{(i)}, \mathbf{x}, p, f, r^{(i)})
\end{align*}
...
\end{equation}
We use $\text{cost}(r, r^{(i)}) = \indicator{r \ne r^{(i)}}$.\footnote{We experimented with recall-oriented training, where errors of omission are assigned a higher cost, but found that while recall increased, overall $F_1$ went down\nss{or: failed to improve?}.}

We learn $\mathbf{w}$ by minimizing the $\ell_2$-regularized average cost on the dataset:
\begin{equation}
\mathbf{w^*} = \argmin_\mathbf{w}{
    \frac{1}{N}\sum_{i = 1}^N{
        \textit{SqHinge}_\mathbf{w}(i)
    } + \frac{1}{2} \lambda \| \mathbf{w} \|_2
}
\end{equation}
