...
have relied exclusively upon full-text annotations in the FrameNet resource 
...
these have been used to train and evaluate contemporary frame-semantic parsers \citep{litkowski,das-14}.
...
We experimented with using more than one level of the hierarchy (e.g., grandparents), but found no improvement in performance despite the increased computation cost owing to the greater number of features.
...
\paragraph{Full-Text.}
For the FT annotations, we use the same FrameNet 1.5 data and train/test splits as \citet{das-14}.
...
SemLink, as noted in \cref{sec:preproc}, is a noisy resource---an observation confirmed by the performance of the (FT+SemLink) model,
which drops $F_1$ by a whopping 11.2 and 16.5 points below the baseline on FT-test and exemplars, respectively. 
The guide features, however, modulate the influence
of the SemLink annotations, giving a minor increase over the baseline.\nss{the two results are presented in the opposite order in the table}
...
On the exemplars-test we observe a similar trend: guide features give a smaller benefit but the (FT+Exemplars) model increases $F_1$ by $\approx$23 $F_1$-points. This contrasts with what we observed for SemLink.
