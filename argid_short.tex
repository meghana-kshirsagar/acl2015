...
partially annotated exemplars from FrameNet.  We observe a 4\% $F_1$ \mk{why percentage for $F_1$? i dont think it is the norm}
...
\mk{It is the latter, but the scoring that uses gold frames does not make much sense if one wants to only evaluate SRL.
If one wants to report the full-system's performance, a frame-id system should be used to get the frames. 
In any case, we do have the numbers using their scoring scheme: 77.12. The revised number from Das et al. after correction is 76.34.}
...
\mk{New text above.. please check. Do we want to quote the result: 70.27 on full-system 
