...
being filled, $r_t$. \nascomment{why subscript t?  notation here should
  be cleaned up and made precise}
...
\nascomment{I cut a footnote here}
...
$\approx$4,000 \nascomment{give exact number as a way for others to
  check that they are replicating correctly; there is no reason to
  obfuscate by giving approximate numbers, it doesn't save space} sentences which had no role annotations.
...
We use the performance \nascomment{vague:  what score?  $F_1$?} on the FT dev set to determine the stopping criterion for optimization.
...
and one using the output of \nascomment{the system described by ?} \citet{hermann-14}, the current state-of-the-art in
...
instances in Table \ref{tbl:results}, for all approaches. % that we
                                % tried.
\nascomment{the following sentence is confusing.  is it a claim about
