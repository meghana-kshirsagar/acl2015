...
\title{Improved Frame-Semantic Argument Identification with PropBank,
Exemplars, and Hierarchical Features}
...
An example appears in \cref{fig:harbor-fn}; it will be explained in detail in
\cref{sec:fn}.
...
We address the sparsity of training data by using standard domain
adaptation techniques to introduce supervision from three sources:
\begin{itemize}
  ...
  by using frustratingly simple domain adaptation (\S\ref{sec:frust}), and
...
Our results show that these sources of evidence boost $F_1$ for argument
identification by almost 4 points, and for end-to-end frame-semantic parsing by
...
The \textbf{full-text (FT)} portion of the FrameNet corpus provides about 5,000
sentences annotated for all frames and their arguments.
...
The exemplar sentences formed the basis of early studies of frame-semantic role
labeling
\citep[e.g.,][]{gildea-02,thompson-03,fleischman-03,litkowski-04,kwon-04}.
Thus far though, exemplars have not been exploited successfully to improve
argument identification performance on full-text data, a more realistic task.
...
\S\ref{sec:base_model}%--\ref{sec:learning} 
, as a starting point.
We experiment with %several domain adaptation (DA) techniques, augmenting 
techniques that augment
the model's training data (\cref{sec:frust}) and feature space %$\mathbf{\phi}$
...
For each candidate argument $a \in \textit{spans(\mathbf{x}, p, f)}$ for a role
...
\subsection{Exemplars / Frustratingly Easy}
\label{sec:frust}
...
% \right.
% \]
Let $\mathcal{D}_{ex}$ denote the exemplars training data, and
$\mathcal{D}_{ft}$ denote the full text training data.
For every feature $\phi_i(a, \mathbf{x}, p, f, r)$, we add a new
feature that fires only if $\phi_i$ fires and $\x \in \mathcal{D}_{ft}$:
...
While the impact on full-text accuracy is negligible, 
