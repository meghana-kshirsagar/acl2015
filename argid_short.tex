...
Horizontal lines show the argument spans.
...
\textbf{Inheritance} and \textbf{Subframe} relations will share a common
feature.
%Define $\textit{siblings}(r)=\{r' \mid parents(r') \cap parents(r) \ne
%\emptyset\}$.
% We create a new family of feature templates: 
For each base feature $\phi$ which is conjoined with the role $r$ in the
baseline model ($\phi \wedge \text{"role=r"}$), and for each parent $r^\prime$ of
$r$, we add a new feature that is the base feature conjoined with the parent role,
($\phi \wedge \text{"parent\_role=r'"}$).
...
first uses PropBank as guide features, and the second adds hierarchy %(sibling) 
features.  
...
Each $x$-axis value is one role, sorted by decreasing frequency (the
