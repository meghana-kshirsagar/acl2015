...
in order to integrate evidence from these three sources 
...
identification by almost 4 points, leading to an improvement in end-to-end
frame-semantic parsing of about 1 point.\nss{relationship to SOTA?}
...
and relative to the FT dataset, these contain an order of magnitude more frame annotations over two orders of magnitude 
...
argument identification performance on FT data, a more realistic task.
...
The global classifier chooses a joint assignment of all arguments of a frame,
while respecting the constraints that a role may be assigned to at most one span, and
...
is to train a supervised model on a source domain, make predictions using that
...
We used two types of guide features:
...
Then the squared hinge loss is given by:
\begin{align*}
\lefteqn{\textit{SqHinge}_\mathbf{w}(i) =} \\
&& (\max_{a^\prime}{\{\mathbf{w}^\top \mathbf{\phi}(a^\prime, \mathbf{x}, p, f,
r) + \text{cost}(a^\prime, a)\}} - \\ 
...
$\approx$4,000 sentences which had no role annotations.
...
data.
We also merged spans which were adjacent and had the
same role label.
...
We use the performance on the FT dev set to determine the stopping criterion for optimization.
...
separate the errors made by frame identification;
and one using the output of \citet{hermann-14}, the current state-of-the-art in
frame identification, to demonstrate that our improvements are retained when
...
instances in Table \ref{tbl:results}, for all approaches. % that we tried.
The baseline $F_1$ on argument identification \citep{das-14} adds `pseudo' scores for getting gold frames right. 
Since our focus is on SRL, we report $F_1$ computed over arguments only. 
The first column shows the resource used (FT/Exemplars/PB-SRL) along with the method. 
The marker `$\xrightarrow{\text{guide}}$' refers to the feature augmentation described in \cref{sec:guide},
...
We also show the sizes of the learned models in the column `Millions of features'.
\mk{IMPORTANT: change the features column to refer instantiated features}
...
\paragraph{Combining all resources}: The last two rows of Table \ref{tbl:results} show the performance upon combining the best approaches. 
...
We present a detailed picture of the gains from the additional coverage by analyzing the $F_1$ at the
level of frame-elements i.e role type. Figure \ref{fig:num_inst} shows the frequency of all roles that appear in the FT-test set.
Figure \ref{fig:coolplot} shows the $F_1$ per role-type, for the baseline and the two best models from Table \ref{tbl:results}.
...
Our models achieve the best improvements for the rarer roles and are at par with the baseline on the frequent roles.
