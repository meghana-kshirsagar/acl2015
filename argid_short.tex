...
% Nathan Schneider, Shay B. Cohen, and Noah A. Smith 
 \author{Meghana Kshirsagar$^\ast$ \quad Sam Thomson$^\ast$ \quad
   Nathan Schneider$^\dagger$ \\
  \bf \quad Jaime Carbonell$^\ast$ \quad Noah A. Smith$^\ast$ \quad Chris
   Dyer$^\ast$  \\
 	    $^\ast$School of Computer Science,
 	    Carnegie Mellon University,
 	    Pittsburgh, PA, USA\\
$^\dagger$School of Informatics, University of Edinburgh, Edinburgh,
Scotland, UK \\
	   }
...
\section{Introduction}
 ...
 \blfootnote{$^\ddagger$ Corresponding author: \tt {mkshirsa@cs.cmu.edu}}
...
FrameNet via SemLink 1.2.2c \citep{bonial-13}
hurt performance, likely due to errors and coverage gaps in the mappings.}
\end{itemize}
...
Our code and models are available at \url{http://www.ark.cs.cmu.edu/SEMAFOR/}.
...
\begin{figure}
\includegraphics[width=\columnwidth]{fig/stay-pb.pdf}
\caption{A PropBank-annotated sentence from OntoNotes \citep{ontonotes}. 
...
Clear similarities to the FrameNet annotations in \cref{fig:hold-off-fn} are evident,
though PB uses lexical frames rather than deep frames 
and makes some different decisions about roles (e.g., \texttt{want-v-01} has no analogue 
to \fnr{Focal\_participant}).}
\label{fig:stay-pb}
\end{figure}
...
with an inventory of \textbf{frames} (such as \fnf{Shopping} and \fnf{Scarcity}). %, and \fnf{Shoot\_projectiles}). 
...
were \emph{annotated} at the document level, and train/development/test splits are by document, the frame-semantic parsing 
...
An example with PB annotations is shown in \cref{fig:stay-pb}.
%}
...
} \nascomment{I agree with Sam -- this is not equivalent, and the
  parenthetical is not actually correct.  adding a copy for the other
  domain would give more freedom to capture idiosyncrasies of that
  domain and let the general feature coefficients be more general.
  it's true that there would be no more expressive power that way only
  if we don't regularize, but I'm assuming we do regularize.}
...
For PropBank, we run the SRL system of Illinois Curator 1.1.4
 \citep{punyakanok-08}\footnote{\url{http://cogcomp.cs.illinois.edu/page/software_view/SRL}} 
 on verbs in the full-text data.
...
\arraycolsep=1pt
\left(\hspace{-2pt}\max_{a^\prime} \left\{ \begin{array}{c} \mathbf{w}^\top \boldsymbol{\phi}(a^\prime, \mathbf{x}, p, f,
r) \\ + \boldsymbol{1}\{a^\prime \not = a\} \end{array}\right \} - 
  \mathbf{w}^\top \boldsymbol{\phi}(a, \mathbf{x}, p, f, r)\hspace{-2pt}\right)^2
\end{align*}
\end{itemize}
...
$\lambda$ by tuning on the FT development set, searching over the following values for
$\lambda$:
...
Under our argument-only evaluation, the system of \cite{das-14} gets  59.3\%
$F_1$.
...
% \paragraph{Example.} \Cref{fig:hold-off-fn} is a sentence for which our best model
% fared better than the baseline.
% The baseline predicted no arguments for \textit{hold off}, 
% while the best system matched the gold standard except for the label 
% of the second role---it predicted \fnr{Time} instead of \fnr{End\_point}.\nss{how many attestions of `hold off' in FT training vs. exemplars?}
% For \textit{want}, both systems correctly found \textit{you} as the \fnr{Experiencer}, 
% but only the best system identified \textit{me} as the \fnr{Focal\_participant} (the baseline mislabeled it as \fnr{Event}).\nss{how many `want' \fnr{Focal\_participant}s in FT training vs. exemplars?}
% Even the best system, though, missed the correct \fnr{Event}, \textit{to hold off\ldots}, 
% which is surprising because infinitival \fnr{Event} arguments to \textit{want} and other \fnf{Desiring} verbs
% are well attested in the training data\nss{please check}.
% \nss{Is it not considering \textit{to hold off until I finish July and August} as an argument span? 
% It is not a complete subtree because \textit{me} is the subject of \textit{hold}. 
% If so, this would likely be solved by better syntactic heuristics.}
% %Perhaps the dependency parser's impoverished label set is to blame
% %(infinitival complements as well as adverbial phrase heads are attached with the \texttt{VMOD} relation).
% Both systems correctly parsed the arguments of \textit{finish}.
...
The authors are grateful to Dipanjan Das for his assistance, and to anonymous
reviewers for their helpful feedback.
This research has been supported by the Richard King Mellon Foundation and DARPA grant FA8750-12-2-0342 funded under the
DEFT program.
}
