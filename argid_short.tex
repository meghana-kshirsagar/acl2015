...
have relied exclusively upon a small corpus of full-text annotations %in the FrameNet resource 
...
with the best model achieving nearly a 4-point increase in $F_1$ over the baseline.
\end{abstract}

\section{Introduction}

Sparsity of data resources is a challenge for many computational semantics tasks\st{cite?}.
Frame-semantic parsing \citep{gildea-02,das-14} is a case in point.
...
to evoked \textbf{frames}, and for each evoked frame, finding and labeling its \textbf{argument} phrases 
with frame-specific \textbf{roles}. 
The FrameNet \textbf{full-text} corpus provides about 5,000 sentences annotated for all frames and their arguments; 
these have been used to train and evaluate contemporary frame-semantic parsers \citep{baker-07,das-14}.
...
To get the best of both worlds, we tap into PropBank's vast resources 
...
Further, we look for ways to exploit additional resources in FrameNet itself.
The bulk of sentences in FrameNet---the lexicographic \textbf{exemplars}---are annotated for only one frame per sentence, 
and have thus far not been exploited successfully for the SRL phase of the frame-semantic parsers evaluated on full-text data.
...
\subsection{Matching roles and arguments}

We use the model from SEMAFOR \citep{das-14}, detailed in \S\ref{sec:base_model}--\ref{sec:learning}, as a starting point.
We experiment with %several domain adaptation (DA) techniques, augmenting 
techniques that augment
the model's training data (\S\ref{sec:aug_data}) and feature space %$\mathbf{\phi}$
...
At the core of SEMAFOR's argument identification procedure is a feature-based scoring function 
that assigns a real value to a possible assignment of a candidate argument (span in the sentence) $a$ 
to a role $r$ of the frame ($f$):
\begin{equation}\label{eq:score}
\textit{score}_\mathbf{w}(a \mid \mathbf{x}, p, f, r) = \mathbf{w}^\top \mathbf{\phi}(a, \mathbf{x}, p, f, r)
\end{equation}
Given a (syntactically preprocessed) sentence $\mathbf{x}$, this is evaluated 
...
Binary features $\mathbf{\phi}(\cdot)$ are extracted and weighted by the model parameters $\mathbf{w}$, 
which are learned from FrameNet annotations in the training data.

\section{Leveraging lexicon structure}
...
\subsection{Structure in the FrameNet lexicon}
...
We say that the $\textit{parent}$ of a role is one that has either the \textbf{Inheritance} or \textbf{Subframe} relation to it.\nss{clarify?}
There are 4,138 \textbf{Inheritance} and 589 \textbf{Subframe} links between role types in FrameNet 1.5.
...
\section{Leveraging additional annotations}

In this section, we discuss additional sources of annotations 
that can be used for learning, followed by modeling techniques
to exploit multiple domains and heterogeneous annotations.

\subsection{FrameNet gold annotations}\label{sec:fn-annos}
%\subsection{Full-text Annotations}\label{sec:ft}

% \nss{TODO:move}FrameNet~1.5 defines a structured taxonomy of over 1,000 frames associated with 12,000~English lexical predicates, 
...
To document a given predicate, lexicographers manually select corpus examples and annotate them
...
There are over 140,000 such sentences (an additional 35,000 exemplar sentences contain no overt arguments).
...
The exemplar sentences formed the basis of early studies of frame-semantic role labeling \citep[e.g.,][]{gildea-02,thompson-03,fleischman-03,litkowski-04,kwon-04}.
...
% \right.
% \]
For every feature $\phi_i(a, \mathbf{x}, p, f, r)$, we add a new feature that fires only if $\phi_i$ fires and $\x \in \mathcal{D}_{ft}$: 
$\text{"ft"} \wedge \phi_i(a, \mathbf{x}, p, f, r)$.\finalversion{\nss{why the $i$ subscript? is that for the training instance as above?}}
...
In our case, we treat the full text annotations as our target domain, and experiment with using PropBank and the exemplars data as source domains.
...
the `guide' role label $r_g$ of the span $a$ is the same as its true\st{you mean ``target''?} label $r_t$.
...
these changes decreased training time significantly.\footnote{With SEMAFOR's original features and training data, 
...
For the full-text annotations, we use the same FrameNet 1.5 data and train/test splits as \citet{das-14}.

%\subparagraph{Exemplars.} 
We processed the exemplars data to remove sentences which had no role annotations (under the assumption that these are likely to be
incomplete annotations). Out of a total of 145,838 sentences, $\approx$4,000 had only frame annotations. 
We further removed duplicate sentences that already appear in the full-text data. 
...
values: 10$^{-5}$, 10$^{-7}$, 10$^{-9}$, 10$^{-12}$.
We use the performance on the FT dev set to determine the stopping criterion for optimization. The FT dev set was used 
only to tune parameters and not as part of model construction. 
...
Statistics of both the test sets are given in Table \ref{tbl:datastats}. 
...
Because our focus is on SRL only, we opt to report $F_1$ computed only over arguments. 
...
With the exemplars resource, we find that the guide features give us a modest improvement of 1.1 points on FT-test, 
...
coverage.
Towards this, we present a frame-element i.e role type level analysis comparing the best results with the baseline.
Figure \ref{fig:num_inst} shows the frequency of all roles that appear in the FT-test set.
...
We also show the sizes of the various models in the column `Millions of features'.
