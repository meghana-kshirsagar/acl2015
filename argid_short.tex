...
For every feature $\phi(a, \mathbf{x}, p, f, r)$ 
in the base feature set, 
we add a disjunctive feature:
...
We add an additional feature disjunction for each \textbf{sib} feature:
$\phi^{\text{par+sib}}(a,\mathbf{x},p,f,r) = \phi^{\text{sib}}(a,\mathbf{x},p,f,r) \vee \phi(a, \mathbf{x}, p, f, \textit{parent}(r))$.\footnote{We also experimented with using more than one level of the hierarchy (e.g., grandparents). 
...
and one using the output of %\nascomment{the system described by ?} 
\citet{hermann-14}, the current state-of-the-art in
